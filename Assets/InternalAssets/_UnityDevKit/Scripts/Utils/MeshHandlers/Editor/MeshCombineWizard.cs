...
namespace UnityDevKit.Utils.MeshHandlers
{
    public class MeshCombineWizard : ScriptableWizard
    {
        ...
        void OnWizardCreate()
        {
            ...
            var originalPosition = combineParent.transform.position;
            combineParent.transform.position = Vector3.zero;

            // Locals
            var materialToMeshFilterList =
                new Dictionary<Material, List<MeshFilter>>();
            var combinedObjects = new List<GameObject>();

            var meshFilters = combineParent.GetComponentsInChildren<MeshFilter>();
            ...
            foreach (var meshFilter in meshFilters)
            {
                ...
                {
                    Debug.LogWarning("The Mesh Filter on object " + meshFilter.name +
                ...
                if (materials == null)
                {
                    Debug.LogWarning("The Mesh Renderer on object " + meshFilter.name +
                ...
                if (materials.Length > 1)
                {
                    ...
                    Debug.LogError(
                        "Objects with multiple materials on the same mesh are not supported. Create multiple meshes from this object's sub-meshes in an external 3D tool and assign separate materials to each. Operation cancelled.");
                    return;
                }
            ...
            // Create prefab
            var prefab = PrefabUtility.CreateEmptyPrefab("Assets/" + resultGO.name + ".prefab");
